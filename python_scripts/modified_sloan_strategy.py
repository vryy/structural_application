...
# Reference:
# Dang et al, Evaluate the performance of an accelerated initial stiffness method in 3D finite element analysis
...
class SolvingStrategyPython:
    ...
    def __init__( self, model_part, time_scheme, linear_solver, convergence_criteria, CalculateReactionsFlag, ReformDofSetAtEachStep, MoveMeshFlag, Parameters, space_utils, builder_and_solver ):
        ...
        self.pDx_old = self.space_utils.CreateEmptyVectorPointer()
        ...
        self.Dx_old = (self.pDx_old).GetReference()
...
#        self.system_reorderer = SystemAMDReordererProcess(self.model_part)
        ...
        if not('log_residuum_name' in self.Parameters):
            self.log_residuum = open('residuum_modified_sloan.log', 'w')
        else:
    ...
    def Initialize(self):
        ...
        print("modified_sloan_strategy.Initialize is called")
    ...
    def PerformNewtonRaphsonIteration( self ):
        ...
        self.log_residuum.write('0\t1.0\t' + str(er_0) + '\n')
        ...
        while(it < self.max_iter and converged == False):
            ...
            if it == 0:
                ...
                if it%2 == 0:
                    alpha = alpha_old + (1.0/alpha_old) * self.space_utils.Dot(self.Dx, self.Dx) / self.space_utils.Dot(self.Dx, self.Dx_old)
                    # alpha = alpha_old + alpha_old * self.space_utils.Dot(self.Dx, self.Dx_old) / self.space_utils.Dot(self.Dx, self.Dx)
                    alpha_old = alpha
                else:
                    alpha = 1.0
            ...
            self.iterationCounter = self.iterationCounter + 1
            self.space_utils.Copy(self.Dx, self.Dx_old)
            ...
            if (converged == False)  and (er_ratio < self.erbar):
                converged = True
                print("modified_sloan_strategy.PerformNewtonRaphsonIteration converged when residuum (" + str(er_ratio) + ") reached tolerance " + str(self.erbar))
        ...
        if( it == self.max_iter and converged == False):
            ...
            if('stop_Newton_Raphson_if_not_converged' in self.Parameters):
                ...
                sys.exit("Sorry, my boss does not allow me to continue. The time step did not converge at time step " + str(self.model_part.ProcessInfo[TIME]) + ", it = " + str(it) + ", max_iter = " + str(self.max_iter))
        print("modified_sloan_strategy.PerformNewtonRaphsonIteration converged after " + str(it) + " steps")
    ...
    def ExecuteIteration(self,echo_level,MoveMeshFlag,CalculateNormDxFlag,it,alpha):
        ...
        print("modified_sloan_strategy.ExecuteIteration:InitializeNonLinIteration is called")
...
#            self.space_utils.WriteMatrixMarketMatrix("matrix" + str(self.solveCounter) + "." + str(self.iterationCounter) + ".mm",self.A,False)
    ...
    def FinalizeNonLinIteration(self,ConvergedFlag,MoveMeshFlag):
        if not ConvergedFlag:
            ...
            print("modified_sloan_strategy.FinalizeNonLinIteration:Update is called")
...
#        print("A:" + str(self.A))
        ...
        print("modified_sloan_strategy.FinalizeNonLinIteration:FinalizeNonLinIteration is called")
    ...
    def FinalizeSolutionStep(self,CalculateReactionsFlag):
        ...
        print("modified_sloan_strategy.FinalizeSolutionStep is called")
