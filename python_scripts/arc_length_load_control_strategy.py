...
### In order to use this strategy, the user must provide the set_load_callback function to control the load applying on the model
###
...
#        self.system_reorderer = SystemAMDReordererProcess(self.model_part)
        ...
        if 'arc_length_control_process' not in self.Parameters:
            if self.Parameters['arc_length_constraint_type'] == "sphere":
                ...
                raise Exception("Unknown constraint type " + str(self.Parameters['arc_length_constraint_type']))
        else:
        ...
        self.set_load_callback = None # this needs to be set externally by the user
    ...
    def Initialize(self):
        ...
        print("arc_length_load_control_strategy.Initialize is called")
    ...
    def SetArcLengthControlProcess(self, new_arc_length_control_process):
        ...
        if self.arc_length_control_process.GetConstraint().NeedForceVector() > 0:
            if self.arc_length_control_process.GetConstraint().NeedForceVector() == 1:
                ...
                raise Exception("The requested type of force vector is not supported")
    ...
    def Solve(self):
        ...
        #set the load factor to 1.0 initially
    ...
    def ExecuteIteration(self,echo_level,CalculateNormDxFlag,predictor):
        ...
        print("arc_length_load_control_strategy.ExecuteIteration:InitializeNonLinIteration is called")

        if self.arc_length_control_process.GetConstraint().GetRadius() == 0.0:
        ...
        if self.compute_external_load:
            # record the first load as external load vector
            # it is noted that the current displacement in the system shall be zero, so that Dxl is the same as Dx
            self.space_utils.Copy(self.b, self.b0)
            self.space_utils.Copy(self.Dx, self.Dxl)
            # self.arc_length_control_process.Reset() # set the load factor to zero, the load now can be set by the factor
            self.compute_external_load = False
            if self.arc_length_control_process.GetConstraint().NeedForceVector() > 0:
                if self.arc_length_control_process.GetConstraint().NeedForceVector() == 1:
                    self.arc_length_control_process.GetConstraint().SetForceVector(self.b0)
                else:
                    raise Exception("The requested type of force vector is not supported")
            # print("Unit load: " + str(self.b0))
            # print(self.A)
            # print(self.b)
            # print(self.Dxl)
        else:
        ...
        if self.arc_length_is_active:
            ...
            if predictor == True:
                print("arc_length_load_control_strategy.ExecuteIteration: predicted delta_lambda = " + str(self.arc_length_control_process.GetDeltaLambda()) + ", predicted lambda = " + str(self.arc_length_control_process.GetLambda()))
            else:
                if self.arc_length_control_process.GetSolveMode() == 0:
                    print("arc_length_load_control_strategy.ExecuteIteration: consistent scheme is used")
                elif self.arc_length_control_process.GetSolveMode() == 1:
                    print("arc_length_load_control_strategy.ExecuteIteration: non-consistent scheme is used")
                print("arc_length_load_control_strategy.ExecuteIteration: corrected delta_lambda = " + str(self.arc_length_control_process.GetDeltaLambda()) + ", corrected lambda = " + str(self.arc_length_control_process.GetLambda()))

            # update the system load
            self.set_load_callback(self.model_part, self.arc_length_control_process.GetLambda())
...
#            self.space_utils.WriteMatrixMarketMatrix("matrix" + str(self.solveCounter) + "." + str(self.iterationCounter) + ".mm",self.A,False)
    ...
    def FinalizeNonLinIteration(self,ConvergedFlag,MoveMeshFlag):
        if not ConvergedFlag:
            #perform update
            self.time_scheme.Update(self.model_part,self.builder_and_solver.GetDofSet(),self.A,self.Dx,self.b)
            print("arc_length_load_control_strategy.FinalizeNonLinIteration:Update is called")
    ...
    #        print("A:" + str(self.A))
        ...
        print("arc_length_load_control_strategy.FinalizeNonLinIteration:FinalizeNonLinIteration is called")
    ...
    def Clear(self):
        ...
        print("arc_length_load_control_strategy.Clear is called")
