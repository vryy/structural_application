...
###
### Arc-length strategy employing the consistent scheme
### In order to use this strategy, the user must provide the set_displacement_callback function to control the displacement applying on the model
###
class SolvingStrategyPython:
    ...
    def __init__( self, model_part, time_scheme, linear_solver, convergence_criteria, CalculateReactionsFlag, ReformDofSetAtEachStep, MoveMeshFlag, Parameters, space_utils, builder_and_solver ):
        ...
        self.pDxl = self.space_utils.CreateEmptyVectorPointer()
        self.pb = self.space_utils.CreateEmptyVectorPointer()
        self.pb0 = self.space_utils.CreateEmptyVectorPointer()

        self.A = (self.pA).GetReference()
        self.Dx = (self.pDx).GetReference()
        self.Dxl = (self.pDxl).GetReference()
        self.b = (self.pb).GetReference()
        self.b0 = (self.pb0).GetReference()
...
#        self.system_reorderer = SystemAMDReordererProcess(self.model_part)
        ...
        if self.Parameters['calculate_strain_energy'] == True:
            ...
            self.log_energy = open('strain_energy_arc_length.log', 'w')
        ...
        if 'arc_length_control_process' not in self.Parameters:
            if self.Parameters['arc_length_constraint_type'] == "sphere":
                psi = self.Parameters['arc_length_load_scale_factor']
                s = self.Parameters['arc_length_radius']
                self.arc_length_control_process = ArcLengthControlProcess(ArcLengthSphereConstraint(psi, s))
            elif self.Parameters['arc_length_constraint_type'] == "cylinder":
                s = self.Parameters['arc_length_radius']
                self.arc_length_control_process = ArcLengthControlProcess(ArcLengthCylinderConstraint(s))
            elif self.Parameters['arc_length_constraint_type'] == "cylinder-displacement":
                s = self.Parameters['arc_length_radius']
                self.arc_length_control_process = ArcLengthControlProcess(ArcLengthCylinderUxUyUzConstraint(s))
            elif self.Parameters['arc_length_constraint_type'] == "sphere-displacement":
                psi = self.Parameters['arc_length_load_scale_factor']
                s = self.Parameters['arc_length_radius']
                self.arc_length_control_process = ArcLengthControlProcess(ArcLengthSphereUxUyUzConstraint(psi, s))
        else:
            self.arc_length_control_process = self.Parameters['arc_length_control_process']
        if 'arc_length_solution_scheme' in self.Parameters:
            if self.Parameters['arc_length_solution_scheme'] == 'consistent':
                self.arc_length_control_process.SetSolveMode(0)
            elif self.Parameters['arc_length_solution_scheme'] == 'non-consistent':
                self.arc_length_control_process.SetSolveMode(1)
        self.attached_processes.append(self.arc_length_control_process)
        self.arc_length_control_process_id = len(self.attached_processes) - 1

        self.set_displacement_callback = None # this needs to be set externally by the user
        self.arc_length_is_active = True # flag to turn on/off arc-length control
        self.set_first_displacement = True # flag to set the initial displacement
    ...
    def Initialize(self):
        ...
        if self.model_part.GetBufferSize() < 3:
            self.model_part.SetBufferSize(3)
        self.arc_length_control_process.SetModelPart(self.model_part)
        self.arc_length_control_process.SetBuilderAndSolver(self.builder_and_solver)
        print("arc_length_displacement_control_strategy.Initialize is called")

    #######################################################################
    def SetArcLengthControlProcess(self, new_arc_length_control_process):
        new_arc_length_control_process.Copy(self.arc_length_control_process)
        self.arc_length_control_process = new_arc_length_control_process
        self.attached_processes[self.arc_length_control_process_id] = self.arc_length_control_process

    def DeactivateArcLengthControl(self):
        self.arc_length_is_active = False

    def ActivateArcLengthControl(self):
        self.arc_length_is_active = True

    #######################################################################
    def Solve(self):
        #print self.model_part
        self.solveCounter = self.solveCounter + 1
        if self.set_first_displacement == True:
            self.set_displacement_callback(self.model_part, 0.0) # set the prescribed displacement multiplier to zero initially
            self.set_first_displacement = False
    ...
    def PerformNewtonRaphsonIteration( self ):
        ...
        if( it == self.max_iter and converged == False):
            ...
            if('stop_Newton_Raphson_if_not_converged' in self.Parameters):
                ...
                sys.exit("Sorry, my boss does not allow me to continue. The time step did not converge at time step " + str(self.model_part.ProcessInfo[TIME]) + ", it = " + str(it) + ", max_iter = " + str(self.max_iter))
        print("arc_length_displacement_control_strategy.PerformNewtonRaphsonIteration converged after " + str(it) + " steps")
    ...
    def InitializeSolutionStep(self):
        if(self.builder_and_solver.GetDofSetIsInitializedFlag() == False or self.ReformDofSetAtEachStep == True):
            ...
            self.A = (self.pA).GetReference()
            self.Dx = (self.pDx).GetReference()
            self.space_utils.Copy(self.Dx,self.Dxl)
            self.b = (self.pb).GetReference()
            self.space_utils.Copy(self.b,self.b0)
        if(self.SolutionStepIsInitialized == False):
    ...
    #######################################################################
    def ExecuteIteration(self,echo_level,CalculateNormDxFlag,predictor):
        ...
        print("arc_length_displacement_control_strategy.ExecuteIteration:InitializeNonLinIteration is called")
        ...
        # solve for delta_u_lambda
        self.space_utils.SetToZeroVector(self.b0)
        self.space_utils.UnaliasedAdd(self.b0,1.0,self.time_scheme.GetForceVector())
        self.linear_solver.Solve(self.A,self.Dxl,self.b0)

        # print("At solve, A = " + str(self.A))
        # print("At solve, rhs = " + str(self.b))
        # print("At solve, dx_u = " + str(self.Dx))
        # print("At solve, dx_l = " + str(self.Dxl))

        if self.arc_length_is_active:

            if self.arc_length_control_process.GetConstraint().NeedForceVector() > 0:
                if self.arc_length_control_process.GetConstraint().NeedForceVector() == 1:
                    self.arc_length_control_process.GetConstraint().SetForceVector(self.b0)
                elif self.arc_length_control_process.GetConstraint().NeedForceVector() == 2:
                    self.arc_length_control_process.GetConstraint().SetForceVector(self.time_scheme.GetForceVector2())

            ## invoke the arc-length calculate process
            self.arc_length_control_process.SetPredictor(predictor)
            ## METHOD 1
            # self.arc_length_control_process.SetDeltaUr(self.Dx)
            # self.arc_length_control_process.SetDeltaUl(self.Dxl)
            # self.arc_length_control_process.Execute()
            ## METHOD 2
            self.arc_length_control_process.Execute(self.Dx, self.Dxl)
            ##########################################################

            if predictor == True:
                # print("A: " + str(self.A))
                # print("b0: " + str(self.b0))
                # print("Dxl: " + str(self.Dxl))
                print("arc_length_displacement_control_strategy.ExecuteIteration: predicted delta_lambda = " + str(self.arc_length_control_process.GetDeltaLambda()) + ", predicted lambda = " + str(self.arc_length_control_process.GetLambda()))
            else:
                # print("A: " + str(self.A))
                # print("b: " + str(self.b))
                # print("b0: " + str(self.b0))
                ...
                # print("Dxl: " + str(self.Dxl))
                if self.arc_length_control_process.GetSolveMode() == 0:
                    print("arc_length_displacement_control_strategy.ExecuteIteration: consistent scheme is used")
                elif self.arc_length_control_process.GetSolveMode() == 1:
                    print("arc_length_displacement_control_strategy.ExecuteIteration: non-consistent scheme is used")
                print("arc_length_displacement_control_strategy.ExecuteIteration: corrected delta_lambda = " + str(self.arc_length_control_process.GetDeltaLambda()) + ", corrected lambda = " + str(self.arc_length_control_process.GetLambda()))

            # update the system load
            self.set_displacement_callback(self.model_part, self.arc_length_control_process.GetLambda())
...
#            self.space_utils.WriteMatrixMarketMatrix("matrix" + str(self.solveCounter) + "." + str(self.iterationCounter) + ".mm",self.A,False)
    ...
    def FinalizeNonLinIteration(self,ConvergedFlag,MoveMeshFlag):
        if not ConvergedFlag:
            #perform update
            self.time_scheme.Update(self.model_part,self.builder_and_solver.GetDofSet(),self.A,self.Dx,self.b)
            print("arc_length_displacement_control_strategy.FinalizeNonLinIteration:Update is called")
    ...
    #        print("A:" + str(self.A))
        ...
        print("arc_length_displacement_control_strategy.FinalizeNonLinIteration:FinalizeNonLinIteration is called")
    ...
    def FinalizeSolutionStep(self,CalculateReactionsFlag):
        ...
        print("arc_length_displacement_control_strategy.FinalizeSolutionStep is called")
    ...
    def Clear(self):
        ...
        self.space_utils.ClearVector(self.pDxl)
        self.space_utils.ResizeVector(self.Dxl,0)
        ...
        print("arc_length_displacement_control_strategy.Clear is called")
