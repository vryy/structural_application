...
class SolvingStrategyPython:
    ...
    def __init__( self, model_part, time_scheme, linear_solver, convergence_criteria, CalculateReactionsFlag, ReformDofSetAtEachStep, MoveMeshFlag, Parameters, space_utils, builder_and_solver ):
        ...
        self.dof_util = StructuralApplication.DofUtility()
        self.output_util = StructuralApplication.OutputUtility()
        ...
        self.system_reorderer = KratosMultiphysics.Process()
...
#        self.system_reorderer = SystemAMDReordererProcess(self.model_part)
    ...
    def PerformOneIteration( self ):
        print("time = " + str(self.model_part.ProcessInfo[KratosMultiphysics.TIME]))
        ...
        print("newton_raphson_strategy.PerformOneIteration completed at time = " + str(self.model_part.ProcessInfo[KratosMultiphysics.TIME]))
    ...
    def PerformNewtonRaphsonIteration( self ):
        ...
        if( it == self.max_iter and converged == False):
            print("Iteration did not converge at time step " + str(self.model_part.ProcessInfo[KratosMultiphysics.TIME]))
            if('stop_Newton_Raphson_if_not_converged' in self.Parameters):
                if(self.Parameters['stop_Newton_Raphson_if_not_converged'] == True):
                    sys.exit("Sorry, my boss does not allow me to continue. The time step did not converge at time step " + str(self.model_part.ProcessInfo[KratosMultiphysics.TIME]) + ", it = " + str(it) + ", max_iter = " + str(self.max_iter))
                else:
    ...
    def InitializeSolutionStep(self):
        if(self.builder_and_solver.GetDofSetIsInitializedFlag() == False or self.ReformDofSetAtEachStep == True):
            ...
            self.builder_and_solver.ResizeAndInitializeVectors(self.time_scheme,self.pA,self.pDx,self.pb,self.model_part)
            #updating references
            self.A = (self.pA).GetReference()
            self.Dx = (self.pDx).GetReference()
            self.b = (self.pb).GetReference()
...
#        print("A:" + str(self.A))
        ...
        for node in self.model_part.Nodes:
            if node.IsFixed(KratosMultiphysics.DISPLACEMENT_X):
                curr_disp = node.GetSolutionStepValue(KratosMultiphysics.DISPLACEMENT_X)
                delta_disp = node.GetSolutionStepValue(StructuralApplication.PRESCRIBED_DELTA_DISPLACEMENT_X)
                node.SetSolutionStepValue(KratosMultiphysics.DISPLACEMENT_X, curr_disp + delta_disp)
                node.SetSolutionStepValue(StructuralApplication.PRESCRIBED_DELTA_DISPLACEMENT_X, 0.0) # set the prescribed displacement to zero to avoid update in the second step
            if node.IsFixed(KratosMultiphysics.DISPLACEMENT_Y):
                curr_disp = node.GetSolutionStepValue(KratosMultiphysics.DISPLACEMENT_Y)
                delta_disp = node.GetSolutionStepValue(StructuralApplication.PRESCRIBED_DELTA_DISPLACEMENT_Y)
                node.SetSolutionStepValue(KratosMultiphysics.DISPLACEMENT_Y, curr_disp + delta_disp)
                node.SetSolutionStepValue(StructuralApplication.PRESCRIBED_DELTA_DISPLACEMENT_Y, 0.0) # set the prescribed displacement to zero to avoid update in the second step
            if node.IsFixed(KratosMultiphysics.DISPLACEMENT_Z):
                curr_disp = node.GetSolutionStepValue(KratosMultiphysics.DISPLACEMENT_Z)
                delta_disp = node.GetSolutionStepValue(StructuralApplication.PRESCRIBED_DELTA_DISPLACEMENT_Z)
                node.SetSolutionStepValue(KratosMultiphysics.DISPLACEMENT_Z, curr_disp + delta_disp)
                node.SetSolutionStepValue(StructuralApplication.PRESCRIBED_DELTA_DISPLACEMENT_Z, 0.0) # set the prescribed displacement to zero to avoid update in the second step
