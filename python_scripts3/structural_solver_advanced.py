    ...
    def Initialize(self):
        ...
        if( self.analysis_parameters['analysis_type'] == 0 ):
            ...
            if(self.dissipation_radius >= 0.0 and self.dissipation_radius <= 1.0):
                ...
                self.time_scheme = StructuralApplication.ResidualBasedNewmarkScheme(2, self.dissipation_radius-2.0)
            elif(self.dissipation_radius >= 4.0 and self.dissipation_radius <= 5.0): # Hilber-alpha
                self.time_scheme = StructuralApplication.ResidualBasedNewmarkScheme(3, self.dissipation_radius-4.0)
            else:
            ...
            self.model_part.ProcessInfo.SetValue( StructuralApplication.QUASI_STATIC_ANALYSIS, False )
            if(self.dissipation_radius >= 0.0 and self.dissipation_radius <= 1.0):
                self.time_scheme = StructuralApplication.ResidualBasedNewmarkScheme(self.dissipation_radius)
            elif(self.dissipation_radius >= 2.0 and self.dissipation_radius <= 3.0): # Bossak-alpha
                self.time_scheme = StructuralApplication.ResidualBasedNewmarkScheme(2, self.dissipation_radius-2.0)
            ...
            self.time_scheme = StructuralApplication.ResidualBasedStateBasedThetaScheme(self.dissipation_radius)
